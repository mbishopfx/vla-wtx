-- ================================
-- 📊 ANALYTICS TRACKING SYSTEM (PostgreSQL Fixed + Correct Clients Schema)
-- ================================
...
-- Add Google Ads conversion tracking ID column if it doesn't exist
...
-- ================================
-- 🏢 REAL CLIENT SETUP (Using Correct Column Names)
-- ================================

-- Insert/Update Nissan of Wichita Falls client
INSERT INTO clients (
    id,
    company_name,  -- Correct column name
    business_type,
    contact_email,  -- Correct column name
    contact_phone,  -- Correct column name
    website_url,
    address,
    city,
    state,
    ...
    gen_random_uuid(),
    'Nissan of Wichita Falls',  -- Using company_name
    'dealership',
    ...
    google_analytics_property_id = EXCLUDED.google_analytics_property_id,
    google_ads_customer_id = EXCLUDED.google_ads_customer_id,
    google_ads_conversion_tracking_id = EXCLUDED.google_ads_conversion_tracking_id,
    updated_at = NOW();

-- Set up initial analytics goals for this client
...
BEGIN
    ...
    RAISE NOTICE '📊 Tables created: 6 analytics tables + 4 goals';
    RAISE NOTICE '🏢 Client added: Nissan of Wichita Falls';
    RAISE NOTICE '🎯 Ready to track: GA4(376755287) + Ads(789-139-9350) + AW-696993687';
    RAISE NOTICE '🚀 Next: Add tracking script to website & test analytics!';
END $$; 