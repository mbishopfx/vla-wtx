-- ================================
-- 📊 ANALYTICS TRACKING SYSTEM (PostgreSQL Fixed)
-- ================================
...
-- ================================
-- ⚡ FUNCTIONS & PROCEDURES (same as before)
-- ================================
...
-- ================================
/*
Add this to Nissan of Wichita Falls website <head>:
...
<!-- VLA Custom Analytics -->
<script>
  const VLA_CLIENT_ID = 'nissan-wichita-falls';
  const VLA_API_ENDPOINT = 'https://your-dashboard.vercel.app/api/analytics/track';
  ...
  // Send to VLA system
  fetch(VLA_API_ENDPOINT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      client_id: VLA_CLIENT_ID,
  ...
  // Track conversions with AW tag
  function trackConversion(conversionType, value) {
    gtag('event', 'conversion', {
      'send_to': 'AW-696993687/' + conversionType,
      'value': value,
      'currency': 'USD'
    });
